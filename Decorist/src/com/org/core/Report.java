 ...
 ***********************************************************************
 * Functional Test Coverage Description   			   : Generate the Custom Report											   
 ***********************************Header End*********************************************************************************/
...
public class Report implements IReporter {
	...
	protected void writeReporterMessages(List<String> reporterMessages) {
		...
		writer.print("<col width=\"900\">");
		writer.print("<col width=\"150\">");
		writer.print("<col width=\"150\">");
		writer.println("<tr>");
		writer.print("<th># Steps</th>");
		writer.print("<th># Status</th>");
		writer.print("<th># Snapshot</th>");
		writer.println("</tr>");
		while (iterator.hasNext()) {
			String msg = Utils.escapeHtml(iterator.next());
			String snapshot = "NA";
			if (msg.startsWith("Pass ::")) {
				msg = msg.substring(msg.indexOf("::") + 2, msg.length()).trim();
				if (msg.contains("SS_")) {
					snapshot = msg.split("SS_")[1].trim();
					msg = msg.split("SS_")[0].trim();
				}
				writer.print("<tr>");
				writer.print("<td>");
				writer.print(msg);
				...
				writer.print("</td>");
				writer.print("<td>");
				if (!snapshot.contentEquals("NA")) {
					snapshot = snapshot.replaceAll(" ", "%20");
					writer.print("<a href=\\" + snapshot + ">Snapshot</a>");
				} else {
					writer.print("NA");
				}
				writer.print("</td>");
